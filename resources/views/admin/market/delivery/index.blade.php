                    ...
                    <tbody>
                        ...
                        @foreach ($delivery_methods as $method)
                            ...
                            <tr>
                                ...
                                <td>{{ $method->name }}</td>
                                <td>{{ $method->amount }} تومان</td>
                                <td>{{ $method->delivery_time }} - {{ $method->delivery_time_unit }}</td>
                                <td>
                                    <label>
                                        <input id="{{ $method->id }}" onchange="changeStatus({{ $method->id }})" data-url="{{ route('admin.market.delivery.status', $method->id) }}" type="checkbox" @if ($method->status === 1)
                                        checked
                                        @endif>
                                    </label>
                                </td>
                                <td class="width-16-rem text-left">
                                    ...
                                    <form action="{{ route('admin.market.delivery.destroy' ,$method->id) }}" method="post" class="d-inline">
                                        @csrf
                                        {{ method_field('delete') }}

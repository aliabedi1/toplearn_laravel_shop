                        ...
                        <section class="row mt-4">
                            ...
                            <section class="col-md-3">
                                <section class="content-wrapper bg-white p-3 rounded-2 cart-total-price">
                                    ...
                                    <section class="d-flex justify-content-between align-items-center d-none" id="delivery-section">
                                        <p class="text-muted">هزینه ی ارسال</p>
                                        <p class="text-danger fw-bolder"><span id="delivery-price"></span> تومان</p>
                                    </section>

                                    <section class="border-bottom mb-3"></section>
                                    <section class="d-flex justify-content-between align-items-center">
                                        <p class="text-muted">جمع سبد خرید</p>
                                        <p class="fw-bolder"><span id="total_price"></span> تومان</p>
                                    </section>
...
<script>
    ...
    function calculateTotalPrice(deliveryAmount = null)
    {
        $('#total_price').empty();
        ...
        $('#total_price').text(toFarsiDigits(totalPrice));
    }
    ...
    function changePriceWithDeliveryMethod(id) {
        $.ajax({
            url: "{{ route('customer.sales-process.get-delivery-price') }}",
            type: 'POST',
            data: {
                delivery: id,
                _token: "{{ csrf_token() }}",
            ...
            success: function(response) {
                ...
                $('#delivery-price').text(toFarsiDigits(response.amount));

                calculateTotalPrice(response.amount);
